...
%     Nz = 18; % time horizon
    ...
    figure(2); clf; % Torque plots
    a6 = subplot(311)
    h6 = plot(0,0);
    h6.XData = [];
    h6.YData = [];
    ylabel('$\tau_2$ [Nm]', 'Interpreter', 'latex', 'FontSize', 14);
    legend('True $\tau_2$', 'Interpreter', 'latex', 'FontSize', 14);
    title('Hip torque');
    ...
    figure(3); clf; % Foot position and foot velocity plots
    a9 = subplot(311)
    h9 = plot(0,0,0,0);
    h9(1).XData = [];
    h9(2).XData = [];
...
%         h2(2).XData(end+1:end+N) = t;
        ...
        h6.YData(end+1:end+N) = tau2;

        h7.YData(end+1:end+N) = tau3;

        h8.YData(end+1:end+N) = tau4;
    ...
    q1_out= output_data(:,2)';
    q2_out= output_data(:,4)';
    q3_out= output_data(:,8)';
    q4_out= output_data(:,12)';
    dq1_out= output_data(:,3)';
    dq2_out= output_data(:,6)';
    dq3_out= output_data(:,10)';
    dq4_out= output_data(:,14)';
    z_out= [q1_out;q2_out;q3_out;q4_out;dq1_out;dq2_out;dq3_out;dq4_out];
    tau2_out = output_data(:,16);
    tau3_out = output_data(:,17);
    tau4_out = output_data(:,18);
    tau_out=[tau2_out;tau3_out;tau4_out];
    compute_results(z_out,parameters(),tau_out);
end